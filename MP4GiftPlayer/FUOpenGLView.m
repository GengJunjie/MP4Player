...
//    gl_FragColor = vec4(texture2D(inputImageTexture, textureCoordinate).rgb,texture2D(inputImageTexture, textureCoordinate+vec2(0.0, 0.5)).r);//上下
    gl_FragColor = vec4(texture2D(inputImageTexture, textureCoordinate).rgb,texture2D(inputImageTexture, textureCoordinate+vec2(-0.5, 0.0)).r);//左右
}
 );
...
{
    float height   = frameHeight;
//    float height   = frameHeight/2.0;//上下
//    float width  = frameWidth;
    float width  = frameWidth/2.0;//左右
